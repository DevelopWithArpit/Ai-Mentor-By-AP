 ...
 * It can now generate a realistic headshot from a user's photo.
 *
...
const GenerateLinkedInVisualsInputSchema = z.object({
  userImageUri: z.string().optional().describe("The user's own photo as a data URI, to be enhanced into a professional headshot. Expected format: 'data:<mimetype>;base64,<encoded_data>'."),
  ...
  resumeContent: z.string().optional().describe("The user's full resume content (text). This is the primary source of information for generating the cover image and the fallback abstract profile picture."),
...
const GenerateLinkedInVisualsOutputSchema = z.object({
  suggestedProfilePictureUrl: z.string().optional().describe("The data URI of the AI-generated suggestion for a profile picture. Expected format: 'data:image/png;base64,<encoded_data>'. This will be a realistic human face if a user photo is provided, otherwise an abstract design."),
...
const generateLinkedInVisualsFlow = ai.defineFlow(
  ...
  async (input): Promise<GenerateLinkedInVisualsOutput> => {
    ...
    let profilePicPrompt = '';
    
    if (input.userImageUri) {
      // Scenario 1: User uploaded a photo. Generate a realistic, enhanced headshot.
      profilePicPrompt = `You are an expert AI photo editor specializing in creating professional LinkedIn headshots. The user has provided a photo. Your task is to transform it into a high-quality, realistic professional profile picture.
      Key enhancements to make:
      1.  **Background**: Replace the existing background with a clean, professional one. Options include a softly blurred office environment, a solid neutral color (like gray, navy blue, or off-white), or a simple, modern architectural background.
      2.  **Lighting**: Improve the lighting to be flattering and professional, like a studio portrait. Ensure the face is well-lit without harsh shadows.
      3.  **Attire (Subtle Changes)**: If the attire is very casual (e.g., a t-shirt), subtly upgrade it to business casual (e.g., a collared shirt, a blouse, or a blazer). Do not drastically change the style, just make it more professional.
      4.  **Composition**: Crop the image to a standard headshot (head and shoulders). Ensure the person is centered.
      5.  **Overall Quality**: Enhance sharpness and color balance. Do NOT apply unrealistic filters, change facial features, or make the person unrecognizable. The result must be a realistic, trustworthy, and professional photo of the person provided.
      The final image must be square.`;
    } else {
      // Scenario 2: No user photo. Generate an abstract profile picture based on resume.
    ...
    try {
      console.log(`Generating LinkedIn Profile Picture...`);
      const profilePromptPayload = input.userImageUri 
        ? [{ media: { url: input.userImageUri } }, { text: profilePicPrompt }]
        : [profilePicPrompt];
        
      const {media: profileMedia} = await ai.generate({
        model: 'googleai/gemini-2.0-flash-preview-image-generation',
        prompt: profilePromptPayload,
      ...
      console.error(`Failed to generate LinkedIn Profile Picture:`, error);
    }
    ...
    return {
      ...
      coverImagePromptUsed: input.resumeContent ? coverImagePrompt : "Cover image not generated as no resume content was provided."
    };
  }
);
